...
| ------------------- | ------ | ------- | --------------- | ----------- |
| 성공률              | 100%   | 100%    | CallerRuns 발동 | Tomcat 압박 |
| 응답시간 p95        | < 50ms | < 100ms | 수 초           | 수십 초     |
| Tomcat busy threads | 낮음   | 보통    | 높음            | 위험        |

> CallerRunsPolicy 덕분에 요청 자체는 거부되지 않지만, **Tomcat 스레드가 이미지 처리에 점유되어 다른 모든 API가 느려지는 구조적 문제**가 있다.
