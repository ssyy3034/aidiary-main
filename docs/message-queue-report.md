...
┌──────────────────────────────────────────────────────────┐
│  VPC: my-vpc-01 (10.0.0.0/16)                            │
│  ┌────────────────────────────────────────────────────┐  │
│  │  AZ: ap-northeast-2a                               │  │
│  │  ┌──────────────────────────────────────────────┐  │  │
│  │  │  퍼블릭 서브넷: aidiary-public-1 (10.0.1.0/24) │  │  │
│  │  │                                              │  │  │
│  │  │  ┌────────────────────────────────────────┐  │  │  │
...
보안 그룹 인바운드:
  22 (SSH), 80 (HTTP), 443 (HTTPS), 5001 (face-api), 8080 (Spring Boot)
  ← 전부 0.0.0.0/0 (모든 IP 허용)
```

### 🚨 식별된 보안 및 아키텍처 취약점

#### 1. 보안 그룹: 내부 서비스 포트의 퍼블릭 노출 (Critical)

| 포트     | 서비스        | 문제점                                                        |
| :------- | :------------ | :------------------------------------------------------------ |
| **5001** | face-api (ML) | 외부에서 직접 ML 엔드포인트 호출 가능 → 리소스 고갈 공격(DoS) |
| **8080** | Spring Boot   | 리버스 프록시 없이 WAS 직접 노출 → 보안 헤더/SSL 미적용       |

- **수정 방안**: 5001, 8080 포트의 소스를 `0.0.0.0/0`에서 **VPC 내부 CIDR (`10.0.0.0/16`)**로 제한. 외부 트래픽은 **Nginx 리버스 프록시 (80/443)** 또는 **ALB**를 통해서만 진입하도록 구성.

#### 2. 단일 AZ / 단일 인스턴스 (고가용성 부재)

- `ap-northeast-2a` 한 곳에만 배포 → AZ 장애 시 서비스 전면 중단.
- **수정 방안**: 프로덕션에서는 최소 2개 AZ(`2a` + `2c`)에 분산 배치 + ALB 연동.

#### 3. t3.small의 리소스 한계 (2 vCPU / 2GB RAM)

- 7개 Docker 컨테이너를 동시 구동하면 메모리 부족(OOM Kill) 위험이 매우 높음.
- 특히 `face-worker` (MediaPipe + PyTorch) 컨테이너는 단독으로 1.5GB+ 메모리 사용.
- **수정 방안**: 최소 `t3.medium` (4GB) 이상 권장. ML 워커 분리 배포 시 별도 인스턴스.

#### 4. HTTPS 미구성

- 보안 그룹에 443 포트가 열려 있으나, SSL 인증서 연동(ACM + ALB 또는 Certbot + Nginx) 없이는 의미 없음.
- 사용자 인증 토큰(JWT)이 평문(HTTP)으로 전송되어 **중간자 공격(MITM)** 에 취약.

#### 5. RabbitMQ 관리 콘솔 (15672) 접근 제어 미비

- 보안 그룹에 명시되지 않았지만, Docker 내부에서 15672 포트 매핑 시 외부 접근 가능.
- 기본 계정(`guest/guest`)으로 큐 삭제, 메시지 조회 등 관리 기능 악용 가능.
