...
| :------------------------- | :---------------------------- | :---------------------------------- | :--------------------- |
| **최대 수용 용량 (Capa)**  | **35건** (Pool 10 + Queue 25) | **무제한** (디스크/메모리 한계까지) | 🚀 **무한 확장성**     |
| **처리 임계점 (TPS)**      | **~1.1 req/s** (35건 / 30초)  | **30+ req/s** (측정치 기준)         | **27배 이상**          |
| **p95 응답 시간 (120 VU)** | **30,000ms+ (Tomcat 마비)**   | **4.72ms**                          | **6300배 단축**        |
| **시스템 영향도**          | **Tomcat 스레드 직접 점유**   | **Tomcat 스레드 즉각 반환**         | **서비스 안정성 확보** |

#### 아키텍처별 병목 구간 (Bottleneck)

- **@Async 아키텍처**: 동시에 35건 이상의 요청이 들어오는 순간 `CallerRunsPolicy`가 발동합니다. 이때부터 Tomcat 스레드가 직접 30초짜리 분석 작업을 수행하며, 200개의 Tomcat 스레드가 모두 점유되면 서비스 전체가 응답 불능 상태에 빠집니다.
- **RabbitMQ 아키텍처**: 요청 즉시 0.005초 만에 응답을 반환합니다. 실제 분석은 독립된 Consumer(concurrency=10)가 큐에서 차례대로 처리하므로, 요청이 600개 쌓여도 사용자는 지연 없이 다른 기능을 이용할 수 있습니다.

#### 부하 분산 실측 데이터 (RabbitMQ)

```bash
# 120 VU 테스트 중 큐 상항: Tomcat은 유휴 상태, 큐만 안정적으로 쌓임
