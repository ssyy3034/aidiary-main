...
- **감정 기반 일기**
  ...
  - 감정 분석 및 감정 그래프 시각화
  - 감정 상태에 맞춘 AI 피드백 제공
  - 서버 데이터 페이징 처리 및 페이지네이션 UI 구현


- **AI 캐릭터 생성**
  - 정교한 페르소나 생성 위해 부모의 성격 테스트 진행
  - 부모 얼굴 이미지 파일 기반으로 huggingface api 호출하며 얼굴 특징 json으로 추출
  - 추출한 json을 기반으로 필요한 특징 필터링 후 프롬프팅하여 Dall-e 호출하여 캐릭터 생성
  - 페르소나 생성으로 부여된 성격 기반의 대화(OpenAI API)
